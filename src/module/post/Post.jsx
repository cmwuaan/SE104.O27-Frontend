...
function Post() {
  const { data, error, isLoading } = useGetListRequest('/posts');
  const [selectedIds, setSelectedIds] = useState([]);
  const { trigger: deleteTrigger, isMutating: isDeleting } = useSendDeleteRequest('/posts', selectedIds, 'ids');
  ...
  const columns = [
    ...
    {
      title: <span className="flex">Image</span>,
      dataIndex: 'image_url',
      key: 'image_url',
      width: 100,
      render: (record) => {
        return (
          <img
            src={record || 'https://i.pinimg.com/736x/0d/64/98/0d64989794b1a4c9d89bff571d3d5842.jpg'}
            alt={record}
    ...
    {
      title: <span className="flex">Content</span>,
      dataIndex: 'content',
      key: 'content',
      width: 800,
    },
