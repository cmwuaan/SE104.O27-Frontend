...
function Feedback() {
  const { data, error, isLoading } = useGetListRequest('/feedbacks');
  const [selectedIds, setSelectedIds] = useState([]);
  const { trigger: deleteTrigger, isMutating: isDeleting } = useSendDeleteRequest('/feedbacks', selectedIds, 'ids');
  ...
  const onClickEdit = (record) => {
    navigate(`/feedback/${record.id}/edit`);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record, value) => {
        return (
          <img
            src={value.image_url || 'https://i.pinimg.com/736x/0d/64/98/0d64989794b1a4c9d89bff571d3d5842.jpg'}
            alt={record}
