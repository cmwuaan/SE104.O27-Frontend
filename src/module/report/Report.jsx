...
function Report() {
  const { data, error, isLoading } = useGetListRequest('/reports');
  const [selectedIds, setSelectedIds] = useState([]);
  const { trigger: deleteTrigger, isMutating: isDeleting } = useSendDeleteRequest('/reports', selectedIds, 'ids');
  ...
  const onClickEdit = (record) => {
    navigate(`/report/${record.id}/edit`);
  };
  ...
  const columns = [
    ...
    {
      title: <span className="flex">Type</span>,
      dataIndex: 'type',
      key: 'type',
      width: 100,
      render: (record) => {
    ...
    {
      title: <span className="flex">Description</span>,
      dataIndex: 'description',
      key: 'description',
      width: 300,
    },
    {
      title: <span className="flex">Vote</span>,
      dataIndex: 'vote',
      key: 'vote',
      width: 80,
      render: (record) => {
    ...
    {
      title: <span className="flex">Creator</span>,
      dataIndex: 'role_of_creator',
      key: 'role_of_creator',
      width: 150,
      render: (record) => {
        return record.charAt(0).toUpperCase() + record.slice(1);
