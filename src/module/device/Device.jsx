...
function Device() {
  const { data, error, isLoading } = useGetListRequest('/iot-devices');
  const [selectedIds, setSelectedIds] = useState([]);
  const { trigger: deleteTrigger, isMutating: isDeleting } = useSendDeleteRequest('/iot-devices', selectedIds, 'ids');
  ...
  const onClickEdit = (record) => {
    navigate(`/iot-devices/${record.id}/edit`);
  };
  ...
  const columns = [
    ...
    {
      title: <span className="flex">ID</span>,
      dataIndex: 'id',
      key: 'id',
      width: 400,
    },
    {
      title: <span className="flex">IP</span>,
      dataIndex: 'ip',
      key: 'ip',
      width: 250,
    },
    {
      title: <span className="flex">Air Quality</span>,
      dataIndex: 'air_val',
      key: 'air_val',
      width: 160,
      render: (record) => {
    ...
    {
      title: <span className="flex">Left Bin</span>,
      dataIndex: 'left_status',
      key: 'left_status',
      width: 160,
      render: (record) => {
        if (record === null) return <p>--</p>;
    ...
    {
      title: <span className="flex">Right Bin</span>,
      dataIndex: 'right_status',
      key: 'right_status',
      width: 160,
      render: (record) => {
        if (record === null) return <p>--</p>;
