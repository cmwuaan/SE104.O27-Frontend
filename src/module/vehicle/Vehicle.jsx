...
function Vehicle() {
  const { data, error, isLoading } = useGetListRequest('/vehicles');
  const [selectedIds, setSelectedIds] = useState([]);
  const { trigger: deleteTrigger, isMutating: isDeleting } = useSendDeleteRequest('/vehicles', selectedIds, 'ids');
  ...
  const onClickEdit = (record) => {
    navigate(`/vehicle/${record.id}/edit`);
  };
  ...
  const columns = [
    ...
    {
      title: <span className="flex">License</span>,
